...
import os
import binascii
from aesgcm import decryptMessage
import argparse
import logging
...
def decrypt_item(hexstring, keys):
    ...
    full_node_id_list = [key for key in keys.keys() if (
        key.replace(' ', '').lower()[:2 * short_node_length]
         == short_node_id)]
    ...
    plainText = decryptMessage(hex2, key_squashed, node_id_squashed[0:12])

    return plainText

parser = argparse.ArgumentParser(description='Decode OpenTRV radio frames, V1.0. Requires a keyfile in the same format generated by the production programming rig.')
...
key_map = { item[0]: item[1]
    for item in full_key_info }

node_map = { item[2]: item[1]
    for item in full_key_info }
...
for line in data_in:
    ...
    log.debug("timestamp: {} ipaddress: {} hexstring with {} chars: {}".format(timestamp, ipaddress, len(hexstring), hexstring))
